...
| ASI02 | Tool Misuse — agente usa herramientas fuera de su tarea | Permisos por agente escopeados en `.claude/agents/*.md`. `developer` no tiene `git push`/merge; `architect` no escribe código. |
...
| ASI09 | Human Trust violado — autoaprobación | Ningún agente mergea su propio trabajo. `reviewer` solo recomienda; merge a `main` requiere aprobación humana en GitHub. |
